...
export function SectionHeading({ title, copy }: SectionHeadingProps) {
  return (
    <div className="space-y-3">
      ...
      <h1 className="text-4xl font-black tracking-tight text-slate sm:text-5xl">{title}</h1>
      <p className="max-w-3xl text-lg leading-8 text-slate-600">{copy}</p>
    </div>
  );
}
...
export function DecorativeCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-[2rem] bg-slate p-8 text-white">
      ...
      <div className="relative space-y-5">
        ...
        <p className="leading-8 text-slate-200">{copy}</p>
